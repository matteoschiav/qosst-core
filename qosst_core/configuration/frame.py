...
class FrameSynchronizationConfiguration(BaseConfiguration):
    ...
    def from_dict(self, config: dict) -> None:
        ...
        if not issubclass(self.synchronization_cls, SynchronizationSequence):
            raise InvalidConfiguration(
                f"The synchronization class {synchronization_str} is not a subclass of qosst_core.synchronization.SynchronizationSequence."
            )
    ...
    def __str__(self) -> str:
        ...
        res += f"ZC Root : {self.zc_root}\n"
        res += f"ZC Length : {self.zc_length}\n"
        res += f"MLS Nbits : {self.mls_nbits}\n"
